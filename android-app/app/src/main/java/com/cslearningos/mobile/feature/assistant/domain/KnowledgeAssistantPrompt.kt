...
fun buildKnowledgeAssistantSystemPrompt(
    ...
    val outputRule = when (mode) {
        ...
        AssistantRequestMode.Draft -> "Return a concise, editable Markdown learning note only. Choose the best existing Area from the list below and begin the response with <!-- cs-area: AREA_ID -->. Do not wrap the note in code fences."
    }
    val areaBlock = areas
        .joinToString(separator = "\n") { "- ${it.id}: ${it.name}" }
