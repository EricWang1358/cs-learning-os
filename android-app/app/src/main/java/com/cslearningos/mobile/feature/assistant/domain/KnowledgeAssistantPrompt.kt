...
fun buildKnowledgeAssistantSystemPrompt(
    ...
    val outputRule = objectTarget?.outputRule(areas) ?: when (mode) {
        ...
        AssistantRequestMode.Draft -> "Return Markdown only: no preface, no explanation outside the draft, no code fences. First classify the note against the existing Areas and their example titles. At the top, emit <!-- cs-area: AREA_ID --> and <!-- cs-area-reason: one concrete reason tied to the Area name or examples --> only when one existing Area is a clear fit; otherwise omit both and never invent an Area. Then emit exactly one <!-- cs-title: concise plain-text title --> directive; the title value must be plain text, not Markdown, and must not contain body content. Return the complete revised Markdown working draft, not a summary or a patch. Do not collapse the source into a short summary; preserve definitions, examples, constraints, caveats, and user-provided wording when it matters. Use at least these sections when applicable: ## Core concepts, ## Detailed notes, ## Examples or cases, ## Common mistakes, ## Review cards. In the Review cards section, write one or more :::quiz blocks with :::quiz on its own line, then exactly question:, answer:, and explanation: fields on separate lines, then ::: on its own line so the app can sync them into Review. When no Area is a clear fit, the app will require the user to choose an Area before saving. Only if a short thought is genuinely unrelated to the draft and worth keeping, add <!-- cs-capture: text --> on its own line."
        ...
        AssistantRequestMode.ReviewEvaluation -> "Act as an interviewer. Evaluate the student's answer for correctness, completeness, missing assumptions, and likely follow-up questions. Then include exactly one <!-- cs-review-answer: canonical concise answer --> directive. The app opens that directive as an editable review-card draft; the user must choose an Area and save before it enters Review, so do not claim it has already been saved."
    }
    val areaBlock = areas
        .joinToString(separator = "\n") { area ->
...
private fun AssistantEditTarget.outputRule(areas: List<AssistantAreaOption>): String = when (this) {
    is AssistantEditTarget.Node -> if (id == null) {
        if (areaId == null && areas.isEmpty()) {
            ...
            "Return Markdown only for this new node draft: no preface, no explanation outside the draft, no code fences. At the top, emit a validated existing Area directive only when you recommend an Area, then emit exactly one <!-- cs-title: concise plain-text title --> directive; the title value must be plain text, not Markdown, and must not contain body content. Do not collapse the source into a short summary; preserve definitions, examples, constraints, caveats, and user-provided wording when it matters. Use at least these sections when applicable: ## Core concepts, ## Detailed notes, ## Examples or cases, ## Common mistakes, ## Review cards. In the Review cards section, write one or more :::quiz blocks with :::quiz on its own line, then exactly question:, answer:, and explanation: fields on separate lines, then ::: on its own line."
        }
    } else {
        "Return Markdown only for this existing node revision: no preface, no explanation outside the draft, no code fences. At the top, emit a validated existing Area directive only when you recommend moving it, then emit exactly one <!-- cs-title: concise plain-text title --> directive; the title value must be plain text, not Markdown, and must not contain body content. Return the complete revised Markdown working draft, not a summary or patch. Do not collapse the source into a short summary; preserve definitions, examples, constraints, caveats, and user-provided wording when it matters. Use at least these sections when applicable: ## Core concepts, ## Detailed notes, ## Examples or cases, ## Common mistakes, ## Review cards. In the Review cards section, write one or more :::quiz blocks with :::quiz on its own line, then exactly question:, answer:, and explanation: fields on separate lines, then ::: on its own line. Do not create a second node."
    }
