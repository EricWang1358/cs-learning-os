 ...
 * (area 桶, problem_no) 在非归档行内唯一(partial unique 索引 idx_kg_question_areano_null /
 * idx_kg_question_areano_area, 由 Migration 建立, 见类 KDoc); 序号只增不改。
 */
@Entity(
    tableName = "kg_question",
    indices = [
        // 镜像注解(非真实语义): 真实索引为 Migration 内两条 partial unique
        // (SQLite 3.8 便携改写, 取代 RFC 的 COALESCE 表达式索引, 见 Migration8To9 KDoc 受控偏差 4):
        //   idx_kg_question_areano_null ON (problem_no) WHERE status != 'ARCHIVED' AND area_id IS NULL
        //   idx_kg_question_areano_area ON (area_id, problem_no) WHERE status != 'ARCHIVED' AND area_id IS NOT NULL
        Index(name = "idx_kg_question_areano_null", value = ["problem_no"], unique = true),
        Index(name = "idx_kg_question_areano_area", value = ["area_id", "problem_no"], unique = true),
    ],
)
 ...
 * 活跃唯一性(parent, child, scope_type, scope 问题桶) 且 status != 'REJECTED' 由
 * partial unique 索引 idx_kg_edge_live_global / idx_kg_edge_live_scoped 保证
 * (Migration 建立, SQLite 3.8 便携改写, 见 Migration8To9 KDoc 受控偏差 4)。
 */
@Entity(
    tableName = "kg_edge",
    foreignKeys = [
    ...
    indices = [
        // 镜像注解(非真实语义): 真实索引为 Migration 内两条 partial unique
        // (SQLite 3.8 便携改写, 取代 RFC 的 COALESCE 表达式索引, 见 Migration8To9 KDoc 受控偏差 4):
        //   idx_kg_edge_live_global ON (parent, child, scope_type) WHERE !REJECTED AND scope_qid IS NULL
        //   idx_kg_edge_live_scoped ON (parent, child, scope_type, scope_qid) WHERE !REJECTED AND scope_qid IS NOT NULL
        Index(
            name = "idx_kg_edge_live_global",
            value = ["parent_node_id", "child_node_id", "scope_type"],
            unique = true,
        ),
        Index(
            name = "idx_kg_edge_live_scoped",
            value = ["parent_node_id", "child_node_id", "scope_type", "scope_question_id"],
            unique = true,
        ),
