...
class KgRoomTest {
    ...
    @Test
    fun `partial unique 索引已按便携改写建立(纯列 + WHERE 谓词)`() = runBlocking {
        // 受控偏差 4: RFC 的 COALESCE 表达式索引拆成两条纯列 partial unique 索引
        // (SQLite 3.8 兼容), 两条都必须带 WHERE 且不含表达式键。
        for (name in listOf("idx_kg_question_areano_null", "idx_kg_question_areano_area")) {
            db.query("SELECT sql FROM sqlite_master WHERE name = ?", arrayOf(name)).use { c ->
                assertTrue("missing index $name", c.moveToFirst())
                val sql = c.getString(0)
                assertTrue(sql.contains("WHERE", ignoreCase = true))
                assertTrue(sql.contains("area_id", ignoreCase = true) || name.endsWith("_null"))
            }
        }
        for (name in listOf("idx_kg_edge_live_global", "idx_kg_edge_live_scoped")) {
            db.query("SELECT sql FROM sqlite_master WHERE name = ?", arrayOf(name)).use { c ->
                assertTrue("missing index $name", c.moveToFirst())
    ...
    @Test
    fun `同 area 桶同序号且未归档冲突, 不同 area 或已归档放行`(): Unit = runBlocking {
        val dao = db.kgQuestionDao()
    ...
    fun `visibleEdgesForQuestion 只含 GLOBAL 与本题 LOCAL`() = runBlocking {
        // kg_edge.scope_question_id 是本模块内 FK —— 先登记两棵问题根再挂 LOCAL 边
        db.kgQuestionDao().insert(question("q1", areaId = "a1", problemNo = 1))
        db.kgQuestionDao().insert(question("q2", areaId = "a2", problemNo = 1))
    ...
    @Test
    fun `detachEdge 软删墓碑并投影 DELETE, 缺失边抛 NotFound`(): Unit = runBlocking {
