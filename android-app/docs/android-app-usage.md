...
4. 在 `捕捉` 中保存一条 Capture Slip，选择生成 AI 草稿。AI 只会在你显式发起此操作后访问配置的服务商。
5. 生成结果先作为可编辑的 Markdown 草稿打开。检查标题、Area、正文和题卡后，点击 `Save Markdown` 才会写入知识库；关闭、返回或继续编辑都不会让 AI 自动保存内容。

## 知识助理

知识助理可以基于选定的本地学习资料帮助解释概念、整理笔记草稿或规划复习。发送问题前先确认可用的本地资料和提示内容；AI 回复与建议的放置位置都需要你审阅。助理可生成或更新可编辑草稿，也可按你的操作保存一条 Capture Slip，但不会批量写入笔记、题卡或复习记录。
