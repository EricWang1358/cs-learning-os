...
Dependency direction points toward contracts and domain policy. `:domain:assistant` and `:domain:content` are pure Kotlin. `:application:content` owns typed commands and idempotency policy. `:core:database` owns the Room schema and migrations; `:data:content-room` is the only Node command adapter allowed to combine application commands with Room. `:adapter:model-openai` owns endpoint normalization, request JSON, HTTP, SSE parsing, provider error mapping, and credential redaction. `:app` composes these modules while the legacy UI and compatibility facade are migrated.
