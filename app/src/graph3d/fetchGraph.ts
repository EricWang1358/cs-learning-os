...
/**
 * 计算请求 URL。
 * - 常规: `${apiBase}/api/kg/export3d?root=...&rootIsQuestion=...` (合并后挂载点, RFC §3.6)
...
export function export3dUrl(apiBase: string, root: string, rootIsQuestion: boolean): string {
  ...
  return `${base}/api/kg/export3d?root=${encodeURIComponent(root)}&rootIsQuestion=${rootIsQuestion ? 'true' : 'false'}`;
}
