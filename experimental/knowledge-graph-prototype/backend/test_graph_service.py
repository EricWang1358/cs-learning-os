"""Contract tests for the KnowledgeGraph FastAPI service (RFC-knowledge-graph).
...
from graph_service import create_app
...
def create_question(client, title, **kw):
    r = client.post("/graph/questions", json={"commandId": cmd(), "title": title, **kw})
...
def append_specs(client, qid, specs, scope="GLOBAL", parent=None):
    ...
    r = client.post(f"/graph/questions/{qid}/prerequisites", json=body)
    assert r.status_code == 200, r.text
    return r.json()
...
def verify(client, node, quiz, verdict):
    r = client.post("/graph/verifications",
...
def test_question_tree_multi_layer(client):
    ...
    dp = node_id(snap, "DP")
    r = client.get(f"/graph/nodes/{dp}/subtree")
    assert r.status_code == 200, r.text
    sub = r.json()
...
def test_real_cycle_rejected_with_path(client):
    ...
    r = client.post(f"/graph/questions/{qid}/prerequisites",
    ...
    # 成环 → 整笔事务回滚, 树不变
    snap2 = client.get(f"/graph/questions/{qid}/tree").json()
...
def test_problem_local_isolation(client):
    ...
    ta = client.get(f"/graph/questions/{qa['questionId']}/tree").json()
    tb = client.get(f"/graph/questions/{qb['questionId']}/tree").json()
...
def test_shared_subtree_visible_in_both_trees(client):
    ...
    for q in (qa, qb):
        t = client.get(f"/graph/questions/{q['questionId']}/tree").json()
...
def test_proposal_confirm_materializes_and_expiry_rejected(client):
    ...
    r = client.post("/graph/proposals/prerequisite-chain",
                    json={"nodeId": root,
    ...
    c1 = client.post(f"/graph/proposals/{prop['proposalId']}/confirm",
    ...
    titles = {n["title"] for n in client.get(f"/graph/questions/{qid}/tree").json()["nodes"]}
    assert {"recursion", "dynamic", "programming", "memoization"} <= titles

    # confirm 幂等重放: 同 commandId 返回缓存, 不重复落库
    c2 = client.post(f"/graph/proposals/{prop['proposalId']}/confirm",
                     json={"commandId": "cmd-confirm-a"})
    ...
    c3 = client.post(f"/graph/proposals/{prop['proposalId']}/confirm",
                     json={"commandId": cmd()})
    assert c3.status_code == 409 and c3.json()["error"] == "ProposalNotPending"

    # 过期拒绝 (expiresInSeconds=-1 → 立即过期; 契约默认 now+1h)
    r2 = client.post("/graph/proposals/prerequisite-chain",
                     json={"nodeId": root, "questionText": "graphs bfs",
                           "expiresInSeconds": -1})
    c4 = client.post(f"/graph/proposals/{r2.json()['proposalId']}/confirm",
                     json={"commandId": cmd()})
...
def test_jd_confirm_edited_payload_override_and_shape_validation(client):
    r = client.post("/graph/proposals/jd-decomposition",
    ...
    c = client.post(f"/graph/proposals/{pid}/confirm",
    ...
    assert client.get(f"/graph/questions/{q1}/tree").json()["nodes"][0]["title"] == "Edited Q1"
    prog = client.get("/graph/batches/batch-edited/progress").json()
    assert prog["total"] == 2 and prog["perCategory"]["ALGORITHM"]["total"] == 2

    # 形状校验: 缺 questions → 422 ProposalShapeInvalid
    r2 = client.post("/graph/proposals/jd-decomposition",
                     json={"jdText": "x", "categories": 1, "questionsPerCategory": 1})
    c2 = client.post(f"/graph/proposals/{r2.json()['proposalId']}/confirm",
                     json={"commandId": cmd(),
...
def test_record_verification_suggested_next_and_idempotency(client):
    ...
    r2 = client.post("/graph/verifications",
                     json={"commandId": "cmd-ver-1", "nodeId": root,
                           "quizItemId": "quiz-root", "verdict": "FAIL"})
    assert r2.json()["replayed"] is True
...
def test_diagnose_gap_priority(client):
    ...
    r = client.get("/graph/quizzes/quiz-fail/gap")
    assert r.status_code == 200, r.text
    gap = r.json()
    ...
    leaf = client.get("/graph/quizzes/quiz-leaf/gap").json()
...
def test_bottlenecks_threshold_and_sorting(client):
    ...
    r = client.get("/graph/bottlenecks")
    assert r.status_code == 200, r.text
    items = r.json()["items"]
    ...
    # 门槛提高 → 无入选
    assert client.get("/graph/bottlenecks?minDistinctQuestions=3").json()["items"] == []
...
def test_bottlenecks_shared_non_weak_ancestor(client):
    ...
    r3 = client.get("/graph/bottlenecks?minDistinctQuestions=3").json()["items"]
    assert [i["nodeId"] for i in r3] == [p]
    assert client.get("/graph/bottlenecks?minDistinctQuestions=4").json()["items"] == []
...
def test_jd_decomposition_120_questions_and_progress(client):
    ...
    c = client.post(f"/graph/proposals/{prop['proposalId']}/confirm", json={"commandId": cmd()})
    assert c.status_code == 200, c.text
    res = c.json()
    ...
    batch = prop["batchId"]
    prog = client.get(f"/graph/batches/{batch}/progress").json()
    ...
    tree = client.get(f"/graph/questions/{res['createdQuestionIds'][0]}/tree").json()
...
def test_export3d_contract_and_content_hash(client):
    ...
    r1 = client.get("/graph/export3d", params={"root": qid, "rootIsQuestion": True})
    ...
    p2 = client.get("/graph/export3d", params={"root": qid, "rootIsQuestion": True}).json()
    assert p2["contentHash"] == p1["contentHash"]
    # 图变更 → hash 变更
    append_specs(client, qid, [spec("Greedy")])
    p3 = client.get("/graph/export3d", params={"root": qid, "rootIsQuestion": True}).json()
    assert p3["contentHash"] != p1["contentHash"]
    assert len(p3["nodes"]) == 5

    # reroot 导出: 任意节点当根
    dp = next(n["id"] for n in p3["nodes"] if n["title"] == "DP")
    r4 = client.get("/graph/export3d", params={"root": dp, "rootIsQuestion": False})
...
def test_detach_edge_soft_delete(client):
    ...
    r = client.delete(f"/graph/edges/{edge['edgeId']}", params={"commandId": "cmd-detach-1"})
    assert r.status_code == 200, r.text
    ...
    assert snap2["edges"] == []
    # 幂等重放
    r2 = client.delete(f"/graph/edges/{edge['edgeId']}", params={"commandId": "cmd-detach-1"})
